...
bool progress_callback_bridge(float progress, void *user_data) {
    ProgressCallbackData *data = static_cast<ProgressCallbackData *>(user_data);
    if (!data || !data->jvm || !data->callback_object) {
        return true; // Continue loading if callback data is invalid
    }
    ...
    if (result == JNI_EDETACHED) {
        ...
        result = data->jvm->AttachCurrentThread(&env_ptr, nullptr);
        env = static_cast<JNIEnv*>(env_ptr);
        detach_needed = true;
    }

    if (result != JNI_OK || !env) {
        printf("[llama_jni.cpp] Failed to get JNI environment for progress callback\n");
        fflush(stdout);
        return true; // Continue loading
    }

    // Call the Java callback method
    jboolean should_continue = env->CallBooleanMethod(data->callback_object, data->callback_method, static_cast<jfloat>(progress));

    // Check for exceptions
    if (env->ExceptionCheck()) {
        env->ExceptionClear();
        printf("[llama_jni.cpp] Exception in progress callback\n");
        fflush(stdout);
    }
    ...
    return static_cast<bool>(should_continue);
}
...
JNIEXPORT void JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1backend_1free)(JNIEnv
*env,
jobject thiz
)
{
...
env->
ReleaseStringUTFChars(path_model, path_cstr
);
...
JNIEXPORT jlong
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1load_1from_1file_1with_1progress)(
        JNIEnv * env,
        jobject
thiz,
jstring path_model,
...
{
printf("[llama_jni.cpp] Entered JNI llama_model_load_from_file_with_progress\n");
fflush(stdout);
...
ProgressCallbackData *callback_data = nullptr;
if (progress_callback != nullptr) {
callback_data = new(std::nothrow) ProgressCallbackData;
if (callback_data) {
// Get JavaVM
jint result = env->GetJavaVM(&callback_data->jvm);
if (result != JNI_OK) {
printf("[llama_jni.cpp] Error: Failed to get JavaVM\n");
fflush(stdout);
delete
callback_data;
...
callback_data->
callback_object = env->NewGlobalRef(progress_callback);

// Get the callback method ID (assuming it's named 'onProgress' and takes float, returns boolean)
jclass callback_class = env->GetObjectClass(progress_callback);
callback_data->
callback_method = env->GetMethodID(callback_class, "onProgress", "(F)Z");
env->
DeleteLocalRef(callback_class);

if (callback_data->callback_method == nullptr) {
printf("[llama_jni.cpp] Error: Could not find onProgress method\n");
fflush(stdout);
env->
DeleteGlobalRef(callback_data
->callback_object);
delete
callback_data;
callback_data = nullptr;
} else {
// Set up the callback in model_params
model_params.
progress_callback = progress_callback_bridge;
model_params.
progress_callback_user_data = callback_data;
printf("[llama_jni.cpp] Progress callback set up successfully\n");
fflush(stdout);
}
}
}
...
env->
ReleaseByteArrayElements(buffer, buf_ptr,
0); // 0 = copy back and free
...
static inline bool check_batch_index(JNIEnv * env, llama_batch * batch, jint
index,
...
batch->
n_tokens = static_cast<int32_t>(n_tokens);
}
}
...
llama_batch *batch = get_batch_ptr(env, batch_ptr);
if (
check_batch_index(env, batch, index,
"llama_batch_set_token") && batch->token)
{
batch->token[index] = static_cast
<llama_token>(token);
}
...
if (
check_batch_index(env, batch, index,
"llama_batch_set_pos") && batch->pos)
{
batch->pos[index] = static_cast
<llama_pos>(pos);
}
...
batch->seq_id[index][0] = static_cast
<llama_seq_id>(seq_id);
}
...
if (
check_batch_index(env, batch, index,
"llama_batch_set_logits") && batch->logits)
{
batch->logits[index] = static_cast
<int8_t>(logits);
}
...
env->
ReleaseByteArrayElements(buf_array, buf_ptr,
0);
return static_cast
<jint>(result);
}
...
{
env->
ReleaseIntArrayElements(tokens_array, tokens_ptr, JNI_ABORT
); // Abort as we didn't use tokens
return -3;
}
...
env->
ReleaseByteArrayElements(text_buf_array, text_buf_ptr,
0);    // Copy back changes
...
if (ctx)
{
llama_memory_clear(llama_get_memory(ctx), true); // Use llama_memory_*
}
}
...
if (ctx)
{
llama_memory_seq_keep(llama_get_memory(ctx), seq_id
); // Use llama_memory_*
}
}
...
{
env->
ReleaseStringUTFChars(text, text_cstr
);
return -3;
}
...
env->
ReleaseIntArrayElements(tokens_array, tokens_ptr,
0); // Copy back changes
env->
ReleaseStringUTFChars(text, text_cstr
);
...
return ctx ? static_cast
<jint>(llama_pooling_type(ctx)
) : static_cast
<jint>(LLAMA_POOLING_TYPE_UNSPECIFIED);
}
...
++i)
{
jstring path_jstr = (jstring)
env->
GetObjectArrayElement(paths_array, i
);
if (!path_jstr)
{
...
jstring_refs[i] =
path_jstr; // Store ref
paths_cstr_ptrs[i] = env->
GetStringUTFChars(path_jstr,
nullptr);
...
{
env->
ReleaseStringUTFChars(jstring_refs[i], paths_cstr_ptrs[i]
);
}
if (jstring_refs[i])
{
env->
DeleteLocalRef(jstring_refs[i]);
}
}
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1max_1devices)(JNIEnv * env, jobject
thiz)
{
return
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1supports_1mmap)(JNIEnv * env, jobject
thiz)
{
return
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1supports_1mlock)(JNIEnv * env, jobject
thiz)
{
return
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1supports_1gpu_1offload)(JNIEnv * env, jobject
thiz)
{
return
...
JNIEXPORT jlong
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1get_1kv_1cache_1self)(JNIEnv * env, jobject
thiz,
jlong context_ptr
)
{
llama_context *ctx = reinterpret_cast<llama_context *>(context_ptr);
return ctx ? reinterpret_cast
<jlong>(llama_get_memory(ctx)
) : 0;
}
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1rope_1type)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
const llama_model *model = reinterpret_cast<const llama_model *>(model_ptr);
return model ? static_cast
<jint>(llama_model_rope_type(model)
) : static_cast
<jint>(LLAMA_ROPE_TYPE_NONE);
}

JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1n_1layer)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1n_1head)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1n_1head_1kv)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jfloat
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1rope_1freq_1scale_1train)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jlong
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1size)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jlong
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1n_1params)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1has_1encoder)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1has_1decoder)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1decoder_1start_1token)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1is_1recurrent)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1type)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
if (!model)
return static_cast
<jint>(LLAMA_VOCAB_TYPE_NONE);
const llama_vocab *vocab = llama_model_get_vocab(model);
return vocab ? static_cast
<jint>(llama_vocab_type(vocab)
) : static_cast
<jint>(LLAMA_VOCAB_TYPE_NONE);
}
...
JNIEXPORT jstring
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1get_1text)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
const llama_model *model = reinterpret_cast<const llama_model *>(model_ptr);
if (!model)
...
JNIEXPORT jfloat
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1get_1score)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1get_1attr)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
...
if (!model)
return static_cast
<jint>(LLAMA_TOKEN_ATTR_UNDEFINED);
const llama_vocab *vocab = llama_model_get_vocab(model);
return vocab ? static_cast
<jint>(llama_vocab_get_attr(vocab, token)
) : static_cast
<jint>(LLAMA_TOKEN_ATTR_UNDEFINED);
}
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1is_1eog)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1is_1control)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1eot)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1sep)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1get_1add_1bos)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jboolean
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1get_1add_1eos)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1pre)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1suf)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1mid)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1pad)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1rep)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1vocab_1fim_1sep)(JNIEnv * env, jobject
thiz,
jlong model_ptr
)
{
...
JNIEXPORT jlong
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1state_1get_1size)(JNIEnv * env, jobject
thiz,
jlong context_ptr
)
{
...
env->
ReleaseByteArrayElements(dst_array, dst_ptr, (bytes_copied
> 0) ? 0 : JNI_ABORT); // Copy back if copied > 0
...
env->
ReleaseByteArrayElements(src_array, src_ptr, JNI_ABORT
...
{
env->
ReleaseStringUTFChars(path_session, path_cstr
);
return -1L;
}
...
env->
ReleaseIntArrayElements(tokens_out_array, tokens_out_ptr, success_c
? 0 : JNI_ABORT);

return success_c ? static_cast
<jlong>(n_token_count_out_c)
: -1L;
}
...
{
env->
ReleaseStringUTFChars(filepath, filepath_cstr
);
return 0;
}
jsize array_len = env->GetArrayLength(tokens_array);
...
env->
ReleaseIntArrayElements(tokens_out_array, tokens_out_ptr, (bytes_read
> 0) ? 0 : JNI_ABORT);

return (bytes_read > 0) ? static_cast
<jlong>(n_token_count_out_c)
: -1L;
}
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1n_1threads)(JNIEnv * env, jobject
thiz,
jlong context_ptr
)
{
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1n_1threads_1batch)(JNIEnv * env, jobject
thiz,
jlong context_ptr
)
{
...
JNIEXPORT void JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1set_1embeddings)(JNIEnv
*env,
jobject thiz, jlong
context_ptr,
jboolean embeddings
...
JNIEXPORT void JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1set_1causal_1attn)(JNIEnv
*env,
jobject thiz, jlong
context_ptr,
jboolean causal_attn
)
{
llama_context *ctx = reinterpret_cast<llama_context *>(context_ptr);
if (ctx)
...
JNIEXPORT void JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1set_1warmup)(JNIEnv
*env,
jobject thiz, jlong
context_ptr,
jboolean warmup
...
JNIEXPORT void JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1synchronize)(JNIEnv
*env,
jobject thiz, jlong
context_ptr)
{
...
JNIEXPORT jfloatArray
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1get_1embeddings)(JNIEnv * env, jobject
thiz,
jlong context_ptr
)
{
...
JNIEXPORT jfloatArray
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1get_1embeddings_1ith)(JNIEnv * env, jobject
thiz,
jlong context_ptr, jint
index)
{
...
JNIEXPORT jfloatArray
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1get_1embeddings_1seq)(JNIEnv * env, jobject
thiz,
jlong context_ptr, jint
seq_id)
{
...
JNIEXPORT jstring
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1meta_1val_1str)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jstring
key)
{
llama_model *model = reinterpret_cast<llama_model *>(model_ptr);
if (!model) return nullptr;
...
char buffer[1024];
int32_t result = llama_model_meta_val_str(model, key_str, buffer, sizeof(buffer));
env->
ReleaseStringUTFChars(key, key_str
);
...
JNIEXPORT jint
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1meta_1val_1int)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jstring
key)
{
...
// Try to get as string first, then parse as int
char buffer[64];
int32_t result = llama_model_meta_val_str(model, key_str, buffer, sizeof(buffer));
env->
ReleaseStringUTFChars(key, key_str
...
return static_cast
<jint>(std::atoi(buffer)
);
}

JNIEXPORT jstring
JNICALL
JNI_METHOD_NAME(LLAMA_JNI_PREFIX, llama_1model_1token_1to_1piece)(JNIEnv * env, jobject
thiz,
jlong model_ptr, jint
token)
{
...
char buffer[256];
// Use the correct API signature: vocab, token, buf, length, lstrip, special
int n_bytes = llama_token_to_piece(
        ...
        sizeof(buffer),
        0,    // lstrip = 0
        true  // special = true (include special tokens)
);

if (n_bytes < 0) return nullptr;

return env->
NewStringUTF(std::string(buffer, n_bytes)
.
