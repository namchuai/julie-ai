...
actual class LlamaBinding {
    ...
    actual suspend fun loadModel(
        ...
        loadModel(modelPath, modelLoadParams, modelContextParams, null)
    }
    ...
    actual suspend fun loadModel(
        ...
        ctxPtr = NativeMethods.llama_context_init_from_model(
            modelPtr,
            LlamaContextParams.from(modelContextParams),
        )
        if (ctxPtr == 0L) {
    ...
    actual fun predict(prompt: String): Flow<String> = flow {
        ...
        try {
            ...
            if (promptDecodeResult != 0) {
                logMessage(
                    "llama_decode failed during prompt evaluation (code: $promptDecodeResult) - Batch likely empty/incorrect!",
                    isError = true
                )
            ...
            while (nGen < maxTokensToGenerate) {
                ...
                if (logits == null) {
                    logMessage(
                        "Error: Could not get logits (index: $logitIndexInLastBatch). Stopping generation.",
                        isError = true
                    )
                    break
                }
                ...
                if (newTokenId == -1) {
                    logMessage(
                        "Error: Failed to find max logit. Stopping generation.",
                        isError = true
                    )
                ...
                if (nBytes < 0) {
                    logMessage(
                        "Error: llama_token_to_piece failed (code: $nBytes) for token $newTokenId",
                        isError = true
                    )
                    break
                } else if (nBytes > 0) {
                ...
                if (decodeResult != 0) {
                    logMessage(
                        "Error: llama_decode failed during generation (code: $decodeResult) - Batch likely empty/incorrect!",
                        isError = true
                    )
                    break
                }
        ...
        // stringBuilder.toString() // No longer returning a single string
