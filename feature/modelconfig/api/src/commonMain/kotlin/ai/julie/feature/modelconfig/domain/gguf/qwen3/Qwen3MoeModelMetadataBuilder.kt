package ai.julie.feature.modelconfig.domain.gguf.qwen3

import ai.julie.feature.modelconfig.domain.gguf.GgufMetadata
import ai.julie.feature.modelconfig.domain.gguf.GgufMetadataBuilder
...
class Qwen3MoeModelMetadataBuilder : GgufMetadataBuilder {
    val architecture: SupportedArchitecture = SupportedArchitecture.QWEN3MOE
    ...
    override fun setField(key: String, value: Any): GgufMetadataBuilder {
        when (key) {
            // Mandatory fields
            QuantizationVersion.Companion.KEY -> quantizationVersion = (value as? Number)?.toLong()?.toUInt()?.let {
                QuantizationVersion(it)
            }
            Alignment.Companion.KEY -> alignment = (value as? Number)?.toLong()?.toUInt()?.let {
                Alignment(
                    it
                )
            }
            ContextLength(architecture, 0u).key -> contextLength =
                (value as? Number)?.toLong()?.toULong()?.let { ContextLength(architecture, it) }

            EmbeddingLength(architecture, 0u).key -> embeddingLength =
                (value as? Number)?.toLong()?.toULong()?.let { EmbeddingLength(architecture, it) }

            BlockCount(architecture, 0u).key -> blockCount =
                (value as? Number)?.toLong()?.toULong()?.let { BlockCount(architecture, it) }
            FeedForwardLength(architecture, 0u).key -> feedForwardLength =
                (value as? Number)?.toLong()?.toULong()?.let { FeedForwardLength(architecture, it) }

            HeadCount(architecture, 0u).key -> headCount =
                (value as? Number)?.toLong()?.toULong()?.let { HeadCount(architecture, it) }
            LayerNormRmsEpsilon(architecture, 0.0f).key -> layerNormRmsEpsilon =
                (value as? Number)?.toFloat()?.let { LayerNormRmsEpsilon(architecture, it) }

            DimensionCount(architecture, 0u).key -> ropeDimensionCount =
                (value as? Number)?.toLong()?.toULong()?.let { DimensionCount(architecture, it) }

            // General Information fields
            Author.Companion.KEY -> author = (value as? String)?.let { Author(it) }
            Basename.Companion.KEY -> basename = (value as? String)?.let { Basename(it) }
            Datasets.Companion.KEY -> datasets = (value as? List<*>)?.filterIsInstance<String>()?.let {
                Datasets(
                    it
                )
            }
            Description.Companion.KEY -> description = (value as? String)?.let { Description(it) }
            Doi.Companion.KEY -> doi = (value as? String)?.let { Doi(it) }
            FileType.Companion.KEY -> fileType = (value as? Number)?.toLong()?.toUInt()?.let { FileTypeValue.entries.find { ft -> ft.value == it } }?.let {
                FileType(it)
            }
            FineTune.Companion.KEY -> fineTune = (value as? String)?.let { FineTune(it) }
            Languages.Companion.KEY -> languages = (value as? List<*>)?.filterIsInstance<String>()?.let {
                Languages(
                    it
                )
            }
            License.Companion.KEY -> license = (value as? String)?.let { License(it) }
            LicenseLink.Companion.KEY -> licenseLink = (value as? String)?.let { LicenseLink(it) }
            LicenseName.Companion.KEY -> licenseName = (value as? String)?.let { LicenseName(it) }
            Name.Companion.KEY -> name = (value as? String)?.let { Name(it) }
            Organization.Companion.KEY -> organization = (value as? String)?.let { Organization(it) }
            QuantizedBy.Companion.KEY -> quantizedBy = (value as? String)?.let { QuantizedBy(it) }
            RepoUrl.Companion.KEY -> repoUrl = (value as? String)?.let { RepoUrl(it) }
            SizeLabel.Companion.KEY -> sizeLabel = (value as? String)?.let { SizeLabel(it) }
            Tags.Companion.KEY -> tags = (value as? List<*>)?.filterIsInstance<String>()?.let {
                Tags(
                    it
                )
            }
            Url.Companion.KEY -> url = (value as? String)?.let { Url(it) }
            Uuid.Companion.KEY -> uuid = (value as? String)?.let { Uuid(it) }
            Version.Companion.KEY -> version = (value as? String)?.let { Version(it) }

            // Source Information fields
            BaseModelCount.Companion.KEY -> baseModelCount = (value as? Number)?.toLong()?.toUInt()?.let {
                BaseModelCount(it)
            }
            SourceDoi.Companion.KEY -> sourceDoi = (value as? String)?.let { SourceDoi(it) }
            SourceRepoUrl.Companion.KEY -> sourceRepoUrl = (value as? String)?.let {
                SourceRepoUrl(
                    it
                )
            }
            SourceUrl.Companion.KEY -> sourceUrl = (value as? String)?.let { SourceUrl(it) }
            SourceUuid.Companion.KEY -> sourceUuid = (value as? String)?.let { SourceUuid(it) }

            // Optional LLM fields
            UseParallelResidual(architecture, false).key -> useParallelResidual = (value as? Boolean)?.let {
                UseParallelResidual(architecture, it)
            }
            TensorDataLayout(architecture, "").key -> tensorDataLayout = (value as? String)?.let {
                TensorDataLayout(architecture, it)
            }
            ExpertCount(architecture, 0u).key -> expertCount = (value as? Number)?.toLong()?.toUInt()?.let {
                ExpertCount(architecture, it)
            }
            ExpertUsedCount(architecture, 0u).key -> expertUsedCount = (value as? Number)?.toLong()?.toUInt()?.let {
                ExpertUsedCount(architecture, it)
            }

            // Attention properties
            HeadCountKv(architecture, 0u).key -> headCountKv = (value as? Number)?.toLong()?.toULong()?.let {
                HeadCountKv(architecture, it)
            }
            MaxAlibiBias(architecture, 0.0f).key -> maxAlibiBias = (value as? Number)?.toFloat()?.let {
                MaxAlibiBias(architecture, it)
            }
            ClampKqv(architecture, 0.0f).key -> clampKqv = (value as? Number)?.toFloat()?.let {
                ClampKqv(architecture, it)
            }
            LayerNormEpsilon(architecture, 0.0f).key -> layerNormEpsilon = (value as? Number)?.toFloat()?.let {
                LayerNormEpsilon(architecture, it)
            }
            KeyLength(architecture, 0u).key -> keyLength = (value as? Number)?.toLong()?.toUInt()?.let {
                KeyLength(architecture, it)
            }
            ValueLength(architecture, 0u).key -> valueLength = (value as? Number)?.toLong()?.toUInt()?.let {
                ValueLength(architecture, it)
            }

            // RoPE properties
            FreqBase(architecture, 0.0f).key -> ropeFreqBase = (value as? Number)?.toFloat()?.let {
                FreqBase(architecture, it)
            }
            Scale(architecture, 0.0f).key -> ropeScaleLinear = (value as? Number)?.toFloat()?.let {
                Scale(architecture, it)
            }
            ...
            ).key -> ropeScalingType = (value as? String)?.let { typeStr ->
                val ropeType = when (typeStr.lowercase()) {
                    "linear" -> RopeScalingType.LINEAR
                    "yarn" -> RopeScalingType.YARN
                    else -> RopeScalingType.NONE
                }
                Type(architecture, ropeType)
            }
            Factor(architecture, 0.0f).key -> ropeScalingFactor = (value as? Number)?.toFloat()?.let {
                Factor(architecture, it)
            }
            OriginalContextLength(architecture, 0u).key -> ropeScalingOriginalContextLength = (value as? Number)?.toLong()?.toUInt()?.let {
                OriginalContextLength(architecture, it)
            }
            FineTuned(architecture, false).key -> ropeScalingFineTuned = (value as? Boolean)?.let {
                FineTuned(architecture, it)
            }

            // SSM properties
            ConvKernel(architecture, 0u).key -> ssmConvKernel = (value as? Number)?.toLong()?.toUInt()?.let {
                ConvKernel(architecture, it)
            }
            InnerSize(architecture, 0u).key -> ssmInnerSize = (value as? Number)?.toLong()?.toUInt()?.let {
                InnerSize(architecture, it)
            }
            StateSize(architecture, 0u).key -> ssmStateSize = (value as? Number)?.toLong()?.toUInt()?.let {
                StateSize(architecture, it)
            }
            TimeStepRank(architecture, 0u).key -> ssmTimeStepRank = (value as? Number)?.toLong()?.toUInt()?.let {
                TimeStepRank(architecture, it)
            }

            // Tokenizer properties
            Model.Companion.KEY -> tokenizerModel = (value as? String)?.let { modelStr ->
            ...
            ChatTemplate.Companion.KEY -> chatTemplate = (value as? String)?.let { ChatTemplate(it) }
            Tokens.Companion.KEY -> tokens = (value as? List<*>)?.filterIsInstance<String>()?.let {
                Tokens(
                    it
                )
            }
            ...
            BosToken.Companion.KEY -> bosToken = (value as? String)?.let { BosToken(it) }
            EosToken.Companion.KEY -> eosToken = (value as? String)?.let { EosToken(it) }
            PaddingToken.Companion.KEY -> paddingToken = (value as? String)?.let { PaddingToken(it) }
            SeparatorToken.Companion.KEY -> separatorToken = (value as? String)?.let {
                SeparatorToken(
                    it
                )
            }
            UnknownToken.Companion.KEY -> unknownToken = (value as? String)?.let { UnknownToken(it) }

            // Token IDs (for resolution)
            BosTokenId.Companion.KEY -> bosTokenId = (value as? Number)?.toLong()?.toUInt()?.let {
                BosTokenId(
                    it
                )
            }
            EosTokenId.Companion.KEY -> eosTokenId = (value as? Number)?.toLong()?.toUInt()?.let {
                EosTokenId(
                    it
                )
            }
            PaddingTokenId.Companion.KEY -> paddingTokenId = (value as? Number)?.toLong()?.toUInt()?.let {
                PaddingTokenId(it)
            }
            SeparatorTokenId.Companion.KEY -> separatorTokenId = (value as? Number)?.toLong()?.toUInt()?.let {
                SeparatorTokenId(it)
            }
            UnknownTokenId.Companion.KEY -> unknownTokenId = (value as? Number)?.toLong()?.toUInt()?.let {
                UnknownTokenId(it)
            }
...
//        if (ropeDimensionCount == null) missingFields.add("llama.rope.dimension_count")
        ...
        val resolvedBosToken = bosToken ?: resolveToken(bosTokenId?.value, tokensList)?.let {
            BosToken(
                it
            )
        }
        val resolvedEosToken = eosToken ?: resolveToken(eosTokenId?.value, tokensList)?.let {
            EosToken(
                it
            )
        }
        val resolvedPaddingToken = paddingToken ?: resolveToken(paddingTokenId?.value, tokensList)?.let {
            PaddingToken(
                it
            )
        }
        val resolvedSeparatorToken = separatorToken ?: resolveToken(separatorTokenId?.value, tokensList)?.let {
            SeparatorToken(
                it
            )
        }
        val resolvedUnknownToken = unknownToken ?: resolveToken(unknownTokenId?.value, tokensList)?.let {
            UnknownToken(
                it
            )
        }

        return Qwen3MoeModelMetadata(
    ...
    companion object {
         ...
         */
        fun create(): Qwen3MoeModelMetadataBuilder = Qwen3MoeModelMetadataBuilder()
    }
}