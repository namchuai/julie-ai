...
data class Qwen3MoeModelMetadata(
    ...
    override val architecture: Architecture = Architecture(value = SupportedArchitecture.QWEN3MOE)
    ...
    companion object {
        fun fromDocument(document: Document): Qwen3MoeModelMetadata {
            val qwen3moeArch = SupportedArchitecture.QWEN3MOE
            ...
            return Qwen3MoeModelMetadata(
                ...
                contextLength = ContextLength(
                    qwen3moeArch,
                    document.getLong("${qwen3moeArch.value}.context_length").toULong()
                ),
                embeddingLength = EmbeddingLength(
                    qwen3moeArch,
                    document.getLong("${qwen3moeArch.value}.embedding_length").toULong()
                ),
                blockCount = BlockCount(
                    qwen3moeArch,
                    document.getLong("${qwen3moeArch.value}.block_count").toULong()
                ),
                feedForwardLength = FeedForwardLength(
                    qwen3moeArch,
                    document.getLong("${qwen3moeArch.value}.feed_forward_length").toULong()
                ),
                useParallelResidual = document.getString("${qwen3moeArch.value}.use_parallel_residual")
                    ?.let { UseParallelResidual(qwen3moeArch, it.toBoolean()) },
                tensorDataLayout = document.getString("${qwen3moeArch.value}.tensor_data_layout")
                    ?.let { TensorDataLayout(qwen3moeArch, it) },
                expertCount = document.getLong("${qwen3moeArch.value}.expert_count")
                    .takeIf { it != 0L }?.let { ExpertCount(qwen3moeArch, it.toUInt()) },
                expertUsedCount = document.getLong("${qwen3moeArch.value}.expert_used_count")
                    .takeIf { it != 0L }?.let { ExpertUsedCount(qwen3moeArch, it.toUInt()) },

                // Attention-specific properties - Some mandatory for LLAMA
                headCount = HeadCount(
                    qwen3moeArch,
                    document.getLong("${qwen3moeArch.value}.attention.head_count").toULong()
                ),
                headCountKv = document.getLong("${qwen3moeArch.value}.attention.head_count_kv")
                    .takeIf { it != 0L }?.let { HeadCountKv(qwen3moeArch, it.toULong()) },
                maxAlibiBias = document.getFloat("${qwen3moeArch.value}.attention.max_alibi_bias")
                    .takeIf { it != 0f }?.let { MaxAlibiBias(qwen3moeArch, it) },
                clampKqv = document.getFloat("${qwen3moeArch.value}.attention.clamp_kqv")
                    .takeIf { it != 0f }?.let { ClampKqv(qwen3moeArch, it) },
                layerNormEpsilon = document.getFloat("${qwen3moeArch.value}.attention.layer_norm_epsilon")
                    .takeIf { it != 0f }?.let { LayerNormEpsilon(qwen3moeArch, it) },
                layerNormRmsEpsilon = LayerNormRmsEpsilon(
                    qwen3moeArch,
                    document.getFloat("${qwen3moeArch.value}.attention.layer_norm_rms_epsilon")
                ),
                keyLength = document.getLong("${qwen3moeArch.value}.attention.key_length")
                    .takeIf { it != 0L }?.let { KeyLength(qwen3moeArch, it.toUInt()) },
                valueLength = document.getLong("${qwen3moeArch.value}.attention.value_length")
                    .takeIf { it != 0L }?.let { ValueLength(qwen3moeArch, it.toUInt()) },

                // RoPE-specific properties - Some mandatory for LLAMA
                ropeDimensionCount = document.getLong("${qwen3moeArch.value}.rope.dimension_count")
                    .takeIf { it != 0L }?.let { DimensionCount(qwen3moeArch, it.toULong()) },
                ropeFreqBase = document.getFloat("${qwen3moeArch.value}.rope.freq_base")
                    .takeIf { it != 0f }?.let { FreqBase(qwen3moeArch, it) },
                ropeScaleLinear = document.getFloat("${qwen3moeArch.value}.rope.scale_linear")
                    .takeIf { it != 0f }?.let { Scale(qwen3moeArch, it) },

                // RoPE Scaling properties - All optional
                ropeScalingType = document.getString("${qwen3moeArch.value}.rope.scaling.type")
                    ?.let { Type(qwen3moeArch, RopeScalingType.valueOf(it.uppercase())) },
                ropeScalingFactor = document.getFloat("${qwen3moeArch.value}.rope.scaling.factor")
                    .takeIf { it != 0f }?.let { Factor(qwen3moeArch, it) },
                ropeScalingOriginalContextLength = document.getLong("${qwen3moeArch.value}.rope.scaling.original_context_length")
                    .takeIf { it != 0L }?.let { OriginalContextLength(qwen3moeArch, it.toUInt()) },
                ropeScalingFineTuned = document.getBoolean("${qwen3moeArch.value}.rope.scaling.finetuned")
                    .takeIf { document.contains("${qwen3moeArch.value}.rope.scaling.finetuned") }
                    ?.let { FineTuned(qwen3moeArch, it) },

                // SSM-specific properties - All optional
                ssmConvKernel = document.getLong("${qwen3moeArch.value}.ssm.conv_kernel")
                    .takeIf { it != 0L }?.let { ConvKernel(qwen3moeArch, it.toUInt()) },
                ssmInnerSize = document.getLong("${qwen3moeArch.value}.ssm.inner_size")
                    .takeIf { it != 0L }?.let { InnerSize(qwen3moeArch, it.toUInt()) },
                ssmStateSize = document.getLong("${qwen3moeArch.value}.ssm.state_size")
                    .takeIf { it != 0L }?.let { StateSize(qwen3moeArch, it.toUInt()) },
                ssmTimeStepRank = document.getLong("${qwen3moeArch.value}.ssm.time_step_rank")
                    .takeIf { it != 0L }?.let { TimeStepRank(qwen3moeArch, it.toUInt()) },
